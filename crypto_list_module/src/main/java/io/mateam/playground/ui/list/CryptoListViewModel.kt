...
class CryptoListViewModel @Inject constructor(
  ...
  val crypocurrencies: LiveData<PagedList<Cryptocurrency>> = Transformations.switchMap(cryptoQueryResults) { it -> it.data }
  val networkErrors: LiveData<String> = Transformations.switchMap(cryptoQueryResults) { it -> it.networkErrors }
  val loadingStatus: LiveData<LoadingStatus> = Transformations.switchMap(cryptoQueryResults) { it -> it.loadingStatus }
